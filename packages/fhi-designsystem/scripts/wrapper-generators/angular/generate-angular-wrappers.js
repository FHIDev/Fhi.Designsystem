...
const main = ({ manifestPath, outputPath }) => {
  ...
  manifest.modules.forEach(module => {
    ...
    const template = `
      ...
      import '@folkehelseinstituttet/designsystem/${tagName}';
      ...
      export class ${className}AngularWrapper {
        ...
        ${events
          .map(
            event => `
            /** ${event.description || ''} */
            @Output("${event.name}") ${snakeToCamel(event.name)} = new EventEmitter<${event.type.text || 'Event'}>();
            handle${snakeToPascal(event.name)}(event: ${event.type.text || 'Event'}) {
