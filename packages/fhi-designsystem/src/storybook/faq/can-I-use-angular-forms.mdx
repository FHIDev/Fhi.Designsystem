...
<Meta title="Ofte stilte spørsmål/Kan jeg bruke komponentene i Angular Forms?" />
...
Om du bruker Angular uten Angular Forms, f.eks. om du bruker komponentene med Javascript/HTML sin standard `formControl` eller utenfor et `form`, kan du bruke de uten ekstra konfigurasjon.
</div>

Ja du kan bruke FHI Designsystem sine form-komponenter i Angular Forms! 🚀

Angular Forms er litt annerledes enn standard HTML Forms, og krever at du bruker Angular sine `ControlValueAccessor` for å koble komponentene til Angular Forms.

Det finnes 2 måter å gjøre dette på:

#### Valg 1. **Bruk `ngDefaultControl` direktivet**
Dette er den enkleste måten å koble en komponent til Angular

For webkomponenter med oppførsel på linje med `<input type="text">`, e.g `<fhi-text-input>`, kan du bruke Angular sitt `ngDefaultControl` direktiv for å koble komponentene til Angular Forms.

Dette fungerer bra for komponenter som oppfører seg som et standard tekstfelt, og er en enkel måte å få komponentene til å fungere med Angular Forms.
For andre komponenter som ikke er tekstbaserte, som `<fhi-radio>` og `<fhi-checkbox>`, anbefales det å bruke `ControlValueAccessor` direktiver som beskrevet i valg 2.
...
Dette er den anbefalte måten å bruke FHI Designsystem sine form-komponenter i Angular Forms, spesielt for komponenter som ikke er tekstbaserte eller har funksjonalitet utover et standard tekstfelt, som `<fhi-radio>` og `<fhi-checkbox>`
